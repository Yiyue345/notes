...
孩子们，一年之后我终于回来了，直接肘赢这一坨东西

事实上，上面那几个`List`的初始化都省略了一个传入的参数，即元素索引
...
val mutableList = MutableList(20) { index: Int ->
    "Element $index"
}
```

由于这个 lambda 表达式只有一个参数，于是我们可以把参数列表全部省略，然后用`it`来指代这个唯一的参数
#### 字符串处理
...
`vararg`意为 var-arg，即可变长度参数，它的用法长下面这样

```kotlin
fun sumNumbers(vararg numbers: Int): Int = numbers.sum()

sumNumbers(1, 2, 3, 4, 5) // 15
```

一般来讲，这个`vararg`都要放在最后一个，不然就得变成命名参数了

像 Dart 一样，Kotlin 也是有展开操作符的，也就是把列表里面的元素全部展开，也就是下面这样

```kotlin
val numbers = intArrayOf(1, 22, 4)

sumNumbers(*n)
```

为了不发生奇怪的情况，一个函数最多只能有一个`vararg`参数

顺带一提，在 Java 中，可变长度参数长这样：

```java
public int sunNumbers(int... numbers) {
